...
impl<const N: usize> Grid<N>
{
    ...
    ) -> [Cell; N]
    {
        let row = row
            ...
            .filter_map(|(y, n)|
                if y == 0 {
                    clues.left[x - 1] = Some(n); None
                } else if y == N - 1 {
                    clues.right[x - 1] = Some(n); None
                }
                else if n > 0 {
                    Some( Cell::Solved(n) )
                } else {
                    Some( Cell::Pencil((1..=N as Digit).collect()) )
                }
            );
...
impl<const N: usize> Grid<N>
{
    // pub fn cells(&self) -> &[Cell; N.pow()]
    // {
    //     self.cells
    // }

    pub fn rows(&self) -> &[[Cell; N]]
    {
        self.cells.as_chunks().0
    }
}
